...
---
salons_rgn as (select sln.id, sln.code_loreal, brand, sln."name" ||'. '|| sln.address || '. ' || sln.city as salon_name,   sln.city,  slt."name" as salon_type, 
...
to_char(sme.started_at::timestamp at time zone 'UTC','dd.mm.YYYY') as FullDate, smr."name",
smr.duration, 
edu.last_name || ' ' || edu.first_name as educator_name,
(case inte.region_level
    when 3 then 'Director'
    when 4 then 'Reg_technolog'
    when 5 then 'Technolog'
	when 6 then 'Partimer'
	end) as  role_name, edu.technolog_salary_category as stavka, 
(case when sme.business_trip is true then 1 else 0 end)  as trip,
(case when sme.studio_id is not null then 'studio' else 'in_salon' end) as type_place, salon_id,
(case when sme.studio_id is not null then  trc."name" || ' ' || trc.address
	else sln.salon_name  end) as name_place, 
to_char(sme.performed_at::timestamp at time zone 'UTC','dd.mm.YYYY') as PerformedDate,
prt.user_count, smr.description,
(case  when sme.performed_at is not null  then '1' else 0 end) as seminar_closed,
(case  when sme.participants_count = '0' then 0 else 1 end) as Users_Count, sme.id, 
sln.code_loreal,
	smr.id as seminartypeid,
	smr."name", 
	edu.last_name || ' ' || edu.first_name as educator_name, sme.educator_id as educater_id,
	brn.pretty_name, sln.com_mreg_name, inte.city, 
	edu.technolog_salary_category as stavka, 
inte.full_name, 
inte.email, 
inte.mobile_number, 
